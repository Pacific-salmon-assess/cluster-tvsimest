...
Type objective_function<Type>::operator() ()
{
  ...
  vector<Type> Srep(timeSteps), beta(timeSteps), Smax(timeSteps), ll(timeSteps),logbeta(timeSteps);
  ...
  for(int i=1;i<timeSteps;i++){  
    renll+= -dnorm(bdev(i-1),Type(0.0),Type(1.0),true); 
    logbeta(i) =  logbeta(i-1) + bdev(i-1)*sigb;
