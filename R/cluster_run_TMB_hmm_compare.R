...
saveRDS(res, file = "res_hmm_compare.rds")
...
for(n in 1:nrow(pars)){
  localres[[n]]<-tmb_func_comp_smax(path=".",
  a=pars$a[n],
  u=pars$u[n])
}

result1<-do.call(rbind, localresR)





sjobtmb <- slurm_apply(tmb_func_comp_smax, pars, jobname = 'TMBrun_smaxcomp',
                    nodes = 300, cpus_per_node = 1, submit = FALSE,
                    pkgs=c("samEst"),
                    rscript_path = "/gpfs/fs7/dfo/hpcmc/pfm/spfm100/caw001/cluster-tvsimest",
                    libPaths="/gpfs/fs7/dfo/hpcmc/pfm/spfm100/R_4.3_ubuntu2404/x86_64-pc-linux-gnu-library/4.3",
...
res <- get_slurm_out(sjobtmb, outtype = 'table', wait = TRUE)
saveRDS(res, file = "res_smax_compare.rds")
