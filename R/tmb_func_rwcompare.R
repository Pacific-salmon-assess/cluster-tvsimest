...
tmb_func_rw_comp <- function(path=".",a, u) {
  ...
  dfsmax<- data.frame(parameter="smax",
    ...
    dfsig<- data.frame(parameter="sigma",
      ...
      sim=rep(dat$sigma,6),
      median=NA,
      mode=rep(c(if(!is.null(ptva$fail_conv)){NA}else{ptva$sig},
             if(!is.null(ptvac$fail_conv)){NA}else{ptvac$sig},
              if(!is.null(ptvb$fail_conv)){NA}else{ptvb$sig},
              if(!is.null(ptvbc$fail_conv)){NA}else{ptvbc$sig},
              if(!is.null(ptvab$fail_conv)){NA}else{ptvab$sig},
              if(!is.null(ptvabc$fail_conv)){NA}else{ptvabc$sig}),each=nrow(df)),
      convergence=rep(c(ifelse(is.null(ptva$fail_conv), ptva$model$convergence, ptva$fail_conv),
                    ifelse(is.null(ptvac$fail_conv), ptvac$model$convergence, ptvac$fail_conv),
                    ifelse(is.null(ptvb$fail_conv), ptvb$model$convergence, ptvb$fail_conv),
                    ifelse(is.null(ptvbc$fail_conv), ptvbc$model$convergence, ptvbc$fail_conv),
    ...
    dfsiga<- data.frame(parameter="sigma_a",
      ...
      sim=NA,
      median=NA,
      mode=c(if(!is.null(ptva$fail_conv)){NA}else{ptva$siga},
             if(!is.null(ptvac$fail_conv)){NA}else{ptvac$siga},
              if(!is.null(ptvab$fail_conv)){NA}else{ptvab$siga},
              if(!is.null(ptvabc$fail_conv)){NA}else{ptvabc$siga}),
    ...
    dfsigb<- data.frame(parameter="sigma_b",
      ...
      sim=NA,
      median=NA,
      mode=c(if(!is.null(ptvb$fail_conv)){NA}else{ptvb$sigb},
              if(!is.null(ptvbc$fail_conv)){NA}else{ptvbc$sigb},
              if(!is.null(ptvab$fail_conv)){NA}else{ptvab$sigb},
              if(!is.null(ptvabc$fail_conv)){NA}else{ptvabc$sigb}),
  ...
  dfumsy <- data.frame(parameter="umsy",
    ...
    dfaic<- data.frame(parameter="AIC",
                       ...
                       mode=c(ifelse(is.null(ptva$fail_conv), ptva$AICc, NA),
                              ifelse(is.null(ptvaEDF$fail_conv), ptvaEDF$AICc, NA),
                              ifelse(is.null(ptvac$fail_conv), ptvac$AICc, NA),
                              ifelse(is.null(ptvacEDF$fail_conv), ptvacEDF$AICc, NA),
                              ifelse(is.null(ptvb$fail_conv), ptvb$AICc, NA),
                              ifelse(is.null(ptvbEDF$fail_conv), ptvbEDF$AICc, NA),
                              ifelse(is.null(ptvbc$fail_conv), ptvbc$AICc, NA),
                              ifelse(is.null(ptvbcEDF$fail_conv), ptvbcEDF$AICc, NA),
    ...
    dfbic<- data.frame(parameter="BIC",
                       ...
                       median=NA,
                        mode=c(ifelse(is.null(ptva$fail_conv), ptva$BIC, NA),
                              ifelse(is.null(ptvaEDF$fail_conv), ptvaEDF$BIC, NA),
                              ifelse(is.null(ptvac$fail_conv), ptvac$BIC, NA),
                              ifelse(is.null(ptvacEDF$fail_conv), ptvacEDF$BIC, NA),
                              ifelse(is.null(ptvb$fail_conv), ptvb$BIC, NA),
                              ifelse(is.null(ptvbEDF$fail_conv), ptvbEDF$BIC, NA),
                              ifelse(is.null(ptvbc$fail_conv), ptvbc$BIC, NA),
                              ifelse(is.null(ptvbcEDF$fail_conv), ptvbcEDF$BIC, NA),
    ...
    dfedf<- data.frame(parameter="EDF",
                       ...
                       median=NA,
                        mode=c(ifelse(is.null(ptva$fail_conv), 4, NA),
                              ifelse(is.null(ptvaEDF$fail_conv), ptvaEDF$EDF, NA),
                              ifelse(is.null(ptvac$fali_conv), 4, NA),
                              ifelse(is.null(ptvacEDF$fail_conv), ptvacEDF$EDF, NA),
                              ifelse(is.null(ptvb$fail_conv), 4, NA),
                              ifelse(is.null(ptvbEDF$fail_conv), ptvbEDF$EDF, NA),
                              ifelse(is.null(ptvbc$fail_conv), 4, NA),
                              ifelse(is.null(ptvbcEDF$fail_conv), ptvbcEDF$EDF, NA),
