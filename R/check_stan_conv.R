check_stan_conv<- function(stansum){
  ...
  conv_mat <- data.frame(variable=stansum$variable,
    rhat=as.numeric((stansum$rhat-1)>.005),
...
check_tmbstan_conv<- function(stansum){
  ...
  rhat <- sum((stansum[,colnames(stansum)=="Rhat"]-1)>.005)
  ess <- stansum[,colnames(stansum)=="n_eff"]<400
  

  conv_mat <- data.frame(variable=rownames(stansum),
    rhat=as.numeric((stansum[,colnames(stansum)=="Rhat"]-1)>.005),
    ess=as.numeric(ess),
    sumconv=as.numeric((stansum[,colnames(stansum)=="Rhat"]-1)>.005)+as.numeric(ess)
    ) 
